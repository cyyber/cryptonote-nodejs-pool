...
function BlockTemplate(template, isRandomX){
    ...
    if (isQRL) {
        if (template.height < config.hardForkHeight) {
            cnAlgorithm = "cryptonight";
            cnVariant = 1;
            cryptoNight = cnHashing[cnAlgorithm];
            this.isRandomX = false;
        } else if(template.height >= config.hardForkHeight) {
            cnAlgorithm = "randomx";
            cnVariant = 0;
            cryptoNight = cnHashing[cnAlgorithm];
            this.isRandomX = true;
...
function processShare(miner, job, blockTemplate, params){
    ...
    if(isQRL) {
      new Buffer(nonce, 'hex').copy(template, 39);
